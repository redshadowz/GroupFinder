	...
	if not GF_PerCharVariables.CurrentGroup then
		...
		for i=1,getn(GF_PerCharVariables.CurrentGroup) do
			...
			elseif not GF_PerCharVariables.CurrentGroup[GF_PerCharVariables.CurrentGroup[i]][5] then
				...
				if not GF_PerCharVariables.CurrentGroup[GF_PerCharVariables.CurrentGroup[i]][3][UnitName("player")] then
					GF_PerCharVariables.CurrentGroup[GF_PerCharVariables.CurrentGroup[i]][3][UnitName("player")] = { UnitLevel("player"),({UnitClass("player")})[2],0,0,0 }
				end
			end
		end
	end
...
function GF_LoadSettings()
	...
	if GF_SearchButtonHasValues() or GF_PerCharVariables.searchbuttonstext[GF_BUTTONS_LIST["SearchList"][getn(GF_BUTTONS_LIST["SearchList"])][4]] then GF_SearchListDropdown:LockHighlight() GF_SearchListClearButton:Show() end
				...
				elseif IsShiftKeyDown() then
					...
					elseif ChatFrameEditBox:IsVisible() then
						...
						if GF_NextAvailableWhoTime + 1 > time() then 
							DEFAULT_CHAT_FRAME:AddMessage(GF_SENDING_WHO_FOR..name.." - "..ceil(GF_NextAvailableWhoTime - time() + getn(GF_UrgentWhoRequest) * 30)..GF_SECONDS,1,1,0.5)
						else
...
function GF_UpdateDPSMeter()
	...
	GF_DamageMeterFrame:SetHeight(25 + math.min(getn(tempTable),10)*18)
end
...
function GF_SetupDPSMeter()
	if ShaguDPSLoaded or not GF_PerCharVariables.usedpsmeter then
		...
		for _,event in { 'CHAT_MSG_COMBAT_SELF_HITS','CHAT_MSG_COMBAT_PARTY_HITS','CHAT_MSG_COMBAT_FRIENDLYPLAYER_HITS','CHAT_MSG_COMBAT_HOSTILEPLAYER_HITS','CHAT_MSG_SPELL_SELF_DAMAGE','CHAT_MSG_SPELL_PARTY_DAMAGE','CHAT_MSG_SPELL_FRIENDLYPLAYER_DAMAGE',
...
function self:CHAT_MSG_COMBAT_SELF_HITS() -- You hit (target) for (damage)(W)
...
function self:CHAT_MSG_COMBAT_PET_HITS() -- 'Your pet' hits (target) for (damage)(W)
...
function self:CHAT_MSG_SPELL_SELF_DAMAGE() -- Your 'spell' hits (target) for (damage)(W)
...
function self:CHAT_MSG_SPELL_PET_DAMAGE() -- 'Your pet' 'spell' hits (target) for (damage)(W)
...
function self:CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE() -- (target) suffers (damage) 'school' from (source)(W)
...
function self:CHAT_MSG_SPELL_DAMAGESHIELDS_ON_SELF() -- You reflect (damage) 'school' to (target)(W)
...
function self:CHAT_MSG_SPELL_DAMAGESHIELDS_ON_OTHERS() -- (source) reflects (damage) 'school' to (target)(W)
...
function self:CHAT_MSG_SPELL_SELF_BUFF() -- Your 'spell' heals (target) for (healing)(W)
...
function self:CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS() -- You gain (healing) from (target) 'spell'
			...
			if i+j <= tempVal then -- if i+j <= tempVal then
				...
				if GF_WORD_LFM[wordString] and (not GF_LFM_TRIGGER[wordString] or GF_WORD_LEVEL_ZONE[wordTable[i+j+1]] or GF_GROUP_IDS[wordTable[i+j+1]] or GF_GROUP_IDS[wordTable[i-1]] or GF_WORD_LEVEL_ZONE[wordTable[i-1]]) then
					...
					if GF_WORD_LEVEL_ZONE[wordTable[i+j+1]] then
						if not foundQuest[1] or GF_WORD_LEVEL_ZONE[wordTable[i+j+1]] > foundQuest[1] then foundQuest[1] = GF_WORD_LEVEL_ZONE[wordTable[i+j+1]] table.insert(lfmPosition, {i,i+j+1,GF_WORD_LFM[wordString] + .5,true}) foundTradesExclusion = foundTradesExclusion + 1.5 foundGuildExclusion = foundGuildExclusion + .1 else table.insert(lfmPosition, {i,i+j+1,GF_WORD_LFM[wordString],true}) end
					elseif GF_GROUP_IDS[wordTable[i+j+1]] then
						table.insert(lfmPosition, {i,i+j+1,GF_WORD_LFM[wordString] + .5,true}) foundTradesExclusion = foundTradesExclusion + 1.5 foundGuildExclusion = foundGuildExclusion + .1
					elseif GF_WORD_LEVEL_ZONE[wordTable[i-1]] then
						if not foundQuest[1] or GF_WORD_LEVEL_ZONE[wordTable[i-1]] > foundQuest[1] then foundQuest[1] = GF_WORD_LEVEL_ZONE[wordTable[i-1]] table.insert(lfmPosition, {i-1,i+j,GF_WORD_LFM[wordString] + .5,true}) foundTradesExclusion = foundTradesExclusion + 1.5 foundGuildExclusion = foundGuildExclusion + .1 else table.insert(lfmPosition, {i-1,i+j,GF_WORD_LFM[wordString],true}) end
					elseif GF_GROUP_IDS[wordTable[i-1]] then
						table.insert(lfmPosition, {i-1,i+j,GF_WORD_LFM[wordString] + .5,true}) foundTradesExclusion = foundTradesExclusion + 1.5 foundGuildExclusion = foundGuildExclusion + .1
					else
				...
				elseif GF_WORD_LFG[wordString] then
					...
					if GF_GROUP_IDS[wordTable[i+j+1]] then
						table.insert(lfmPosition, {i,i+j+1,GF_WORD_LFG[wordString] + .5}) foundTradesExclusion = foundTradesExclusion + 1.5 foundGuildExclusion = foundGuildExclusion + .1
					elseif GF_GROUP_IDS[wordTable[i-1]] then
						table.insert(lfmPosition, {i-1,i+j,GF_WORD_LFG[wordString] + .5}) foundTradesExclusion = foundTradesExclusion + 1.5 foundGuildExclusion = foundGuildExclusion + .1
					else
...
-- Score Trades separately
	...
	for i=1, getn(lfmPosition) do
		...
		for k=lfmPosition[i][2]+1, tempVal do
			for j=1, getn(groupPosition) do if k == groupPosition[j][1] and lfmPosition[i][2] >= lfmPosition[i][1] then lfs = lfs + 1 if lfe == 0 then lfe = 1 foundTradesExclusion = foundTradesExclusion + 1.5 end if showanyway == true then print(lfmPosition[i][3].." reached group 1(R)") end if GF_LFM_CONNECT_WORDS_AFTER[wordTable[k+1]] then lfs = lfs + GF_LFM_CONNECT_WORDS_AFTER[wordTable[k+1]] if showanyway == true then print(lfmPosition[i][3].." "..wordTable[k+1].."(R) "..GF_LFM_CONNECT_WORDS_AFTER[wordTable[k+1]]) end end k = tempVal + 1 break end end
			if wordTable[k] then
		...
		for k=lfmPosition[i][1]-1, 1, -1 do
			for j=1, getn(groupPosition) do if k == groupPosition[j][2] and lfmPosition[i][2] >= lfmPosition[i][1] then lfs = lfs + 1 if lfe == 0 then lfe = 1 foundTradesExclusion = foundTradesExclusion + 1.5 end if showanyway == true then print(lfmPosition[i][3].." reached group 1(L)") end if GF_LFM_CONNECT_WORDS_BEFORE[wordTable[k-1]] then lfs = lfs + GF_LFM_CONNECT_WORDS_BEFORE[wordTable[k-1]] if showanyway == true then print(lfmPosition[i][3].." "..wordTable[k-1].."(L) "..GF_LFM_CONNECT_WORDS_BEFORE[wordTable[k-1]]) end end k = 0 break end end
			if wordTable[k] then
		...
		for j=1, getn(groupPosition) do -- If groupposition is within the lfmposition, don't count it if GF_LFM_BYPASS or GF_WORD_LEVEL_ZONE or GF_GROUP_IDS
			...
			and not (lfmPosition[i][2] > 1 and (GF_LFM_BYPASS[wordTable[lfmPosition[i][2]-1]..wordTable[lfmPosition[i][2]]] or GF_WORD_LEVEL_ZONE[wordTable[lfmPosition[i][2]-1]..wordTable[lfmPosition[i][2]]] or GF_GROUP_IDS[wordTable[lfmPosition[i][2]-1]..wordTable[lfmPosition[i][2]]])) then
				lfs = lfs + 1 if lfe == 0 then lfe = 1 foundTradesExclusion = foundTradesExclusion + 1.5 end
	...
	if tempVal > 15 then foundTrades = foundTrades + 1 - floor(1.05 ^ (tempVal - 20)*10)/10 if showanyway == true then print((floor(1.05 ^ (tempVal - 20)*10)/10 - 1).." subtract for "..tempVal.." words") end end
		...
		if GF_NumPartyMembers == 1 then
			...
			if not GF_PerCharVariables.groupfinishtimer then
				...
				if groupstosave[1] then
					...
					local wordString = ""
					for i=1, getn(groupstosave) do wordString = wordString..groupstosave[i]..", " end
...
function GF_ApplyFiltersToGroupList(override) -- GroupsFrame functions
	...
	GF_UpdateResults()
	if GF_SearchButtonHasValues() or GF_PerCharVariables.searchbuttonstext[GF_BUTTONS_LIST["SearchList"][getn(GF_BUTTONS_LIST["SearchList"])][4]] then GF_SearchListDropdown:LockHighlight() GF_SearchListClearButton:Show() else GF_SearchListDropdown:UnlockHighlight() GF_SearchListClearButton:Hide() end
end
...
-- whisper, invite, target, ignore, cancel
	...
	if not GF_DropDownMenu.block then
		...
		info.func = function()
			...
			if GF_NextAvailableWhoTime + 1 > time() then 
				DEFAULT_CHAT_FRAME:AddMessage(GF_SENDING_WHO_FOR..GF_DropDownMenu.name.." - "..ceil(GF_NextAvailableWhoTime - time() + getn(GF_UrgentWhoRequest) * 30)..GF_SECONDS,1,1,0.5)
			else
...
function GF_GetGroupWhoButton(frame,id)
	...
	elseif not GF_UrgentWhoRequest[GF_FilteredResultsList[GF_ResultsListOffset+id].op] then
		if GF_NextAvailableWhoTime + 1 > time() then
			DEFAULT_CHAT_FRAME:AddMessage(GF_SENDING_WHO_FOR..GF_FilteredResultsList[GF_ResultsListOffset+id].op.." - "..ceil(GF_NextAvailableWhoTime - time() + getn(GF_UrgentWhoRequest) * 30)..GF_SECONDS,1,1,0.5)
		else
...
function GF_CreateGetWhoDropDownMenu()
	...
	info.func = function()
		...
		if GF_NextAvailableWhoTime + 1 > time() then 
			DEFAULT_CHAT_FRAME:AddMessage(GF_SENDING_WHO_FOR..GF_GetWhoDropDownMenu.name.." - "..ceil(GF_NextAvailableWhoTime - time() + getn(GF_UrgentWhoRequest) * 30)..GF_SECONDS,1,1,0.5)
		else
